...
| INV-08 | 판매중 그룹 탭 (사무실/포이즌 창고 분리) | ✅ | 🔶 | subLabels 정의됨 (리스팅/포이즌보관). 별도 탭 UI는 미구현 |
| INV-09 | 재고 그룹 탭 (위치별) | ✅ | 🔶 | subLabels 정의됨 (입고대기/미등록재고). 별도 탭 UI는 미구현 |
| INV-10 | 발송 그룹 탭 (발송일+운송장별) | ✅ | 🔶 | subLabels 정의됨 (발송중/검수중). 별도 탭 UI는 미구현 |
...
> **인증 제외 전체 이식률: 약 89%** (117개 중 104개 완료 + 5개 부분 구현)
