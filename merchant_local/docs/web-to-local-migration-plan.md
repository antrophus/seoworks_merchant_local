...
| INV-04 | 개인/샘플 필터 | ✅ | ❌ | is_personal, SAMPLE 필터 미구현 |
| INV-05 | 카드 리스트 뷰 (무한 스크롤) | ✅ | ✅ | item_tile + ListView |
| INV-06 | 체크박스 선택 (전체 선택/해제) | ✅ | ✅ | selectionProvider |
| INV-07 | 확장 패널 (이미지, 구매/판매 정보) | ✅ | 🔶 | item_tile에 기본 정보만 표시. 확장 패널 미구현 |
| INV-08 | 판매중 그룹 탭 (사무실/포이즌 창고 분리) | ✅ | ❌ | 미구현 |
| INV-09 | 재고 그룹 탭 (위치별) | ✅ | ❌ | 미구현 |
| INV-10 | 발송 그룹 탭 (발송일+운송장별) | ✅ | ❌ | 미구현 |
| INV-11 | 카메라 조회 (바코드로 아이템 검색) | ✅ | ✅ | barcode_widgets + scan_screen |
| INV-12 | Sticky 헤더 | ✅ | ❌ | 스크롤 시 헤더 고정 미구현 |
...
| 캘린더 | 3 | 0 | 0 | 3 | 0 | 0% |
| 설정 | 5 | 5 | 0 | 0 | 0 | 100% |
| **전체** | **121** | **102** | **2** | **13** | **4** | **87%** |

> **인증 제외 전체 이식률: 약 89%** (117개 중 102개 완료 + 2개 부분 구현)
> **핵심 비즈니스 로직 이식률: ~98%** (상태 머신, 수수료 계산, 구매/판매 CRUD, 바코드/AI 스캔, 벌크 액션 전체)
> **전용 페이지/뷰 이식률: ~90%** (분석/하자/물류/구매/판매 페이지 완료, 캘린더만 미구현)
>
...
### Phase 2-C 태스크
1. **캘린더 뷰** (CAL-01~03) — `table_calendar` 패키지, 월별 이벤트 표시
2. **Google Drive 동기화** — CRDT HLC 충돌 해결, SyncMeta 활용 (가장 복잡, 별도 스프린트 권장)
3. **기타 UI 개선** — receipt_url 입력, POIZON 자동 동기화(WorkManager)

### 잔여 이슈
- **하자/물류/구매 페이지 네비게이션 미연결** — 현재 라우트(`/exceptions`, `/logistics`, `/purchases`)로만 접근 가능. 대시보드 또는 설정에서 진입점 추가 필요
- **AI 스캔 → 기존 상품 매칭 시 사이즈 자동완성 버그** — `_applyPrefill`에서 `_selectProduct`의 비동기 사이즈차트 로딩이 완료되기 전에 `_autoFillEuSize` 호출됨. 원인 파악 완료, 수정 미적용
- **AI 스캔 → 신규 상품일 때 폼 전환** — 기존 상품이 없으면 신규 상품 등록 폼으로 전환하여 브랜드/모델명/모델코드 개별 필드에 AI 결과를 채우는 로직 미구현
- **상품 이미지 저장 로직** — QR 스캔으로 가져온 상품 이미지의 로컬 저장 여부 확인 필요
- **인벤토리 부분 구현 잔여** — 개인/샘플 필터(INV-04), 확장 패널(INV-07), 그룹 탭(INV-08~10), Sticky 헤더(INV-12)
- **대시보드 잔여** — 브랜드 Top6 차트(DASH-03), 캘린더 위젯(DASH-04)
